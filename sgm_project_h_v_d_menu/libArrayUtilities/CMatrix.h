...
//  void upsampleBicubic(int aNewXSize, int aNewYSize);
  ...
  // Read matrix from float3 file
  void readFromFloatFile(char const *filename);
  // Save matrix as text file
  void writeToTXT(const char* aFilename, bool aHeader = true) const;
  // Saves matris in float3 format
  void writeToFloatFile(char const *filename);
...
//readFromFloatFile
template <class T>
void CMatrix<T>::readFromFloatFile(char const *filename)
{
  std::ifstream infile(filename, std::ios::binary);
  char buf[128];
  infile.getline(buf, 128);
  if (std::string(buf) != "float")
      throw std::runtime_error(
          (std::string(filename) + " is no valid float CMatrix").c_str());

  int nDims, dimX, dimY, dimZ = 1;
  infile >> nDims >> dimX >> dimY;
  if (nDims > 3)
      throw std::runtime_error(
          (std::string(filename) + " does not contain 2-D data").c_str());      
  if (nDims > 2) infile >> dimZ;
  if (dimZ != 1)
      throw std::runtime_error(
          (std::string(filename) + " contains 3-D data which cannot be "
           "imported to a scalar CMatrix<float>").c_str());
  
  setSize(dimX, dimY);
  infile.ignore(1); // Extract the remaining newline before data start
  infile.read(reinterpret_cast<char*>(data()), size() * sizeof(float));
}

//writeToFloatFile
template <class T>
void CMatrix<T>::writeToFloatFile(char const *filename)
{
  std::ofstream outfile(filename, std::ios::binary | std::ios::trunc);
  outfile << "float\n2\n" << xSize() << "\n" << ySize() << "\n";
  outfile.write(reinterpret_cast<char*>(data()), size() * sizeof(float));
}
